...
class ENV():
    def __init__(self, Orders_Dataset, Grids, Drivers_Dataset, ALG):
        ...
        self.stats_active_drivers = {}
        self.stats_active_orders = {}

        if ALG == 'km':
            self.mdp_table = None
        elif ALG == 'mdp':
            self.mdp_table = pickle.load(open('./algorithms/result/mdp_table_2kd_05o_3', 'rb'))
        else:
            self.mdp_table = Policy(5 + 18, 1)
            self.mdp_table.load_state_dict(torch.load('./algorithms/result/grc_model_1'))
    ...
    def step(self, time):
        ...
        #count driver num
        count_drivers_num(self.active_drivers, self.stats_active_drivers, time)
        
        new_orders_num = load_new_orders(self, time)

        #count order num
        count_orders_num(self.active_orders, self.stats_active_orders, time)

        revenue, matched_orders_num = match(self.active_orders, self.active_drivers, self.inactive_drivers, self.Grids, self.matched_orders, MDP_TABLE=self.mdp_table)
...
if __name__ == "__main__":
    ...
    for date_idx in range(4, 7, 1):
        ...
        env = ENV(Orders_Dataset, Grids, Drivers_Dataset, ALG)
        ...
        pickle.dump(env.stats_active_drivers, open('./result/stats_active_drivers'+str(date_idx)+ALG, 'wb+'))
        pickle.dump(env.stats_active_orders, open('./result/stats_active_orders'+str(date_idx)+ALG, 'wb+'))
